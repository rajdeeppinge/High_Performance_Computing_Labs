...
void writePPMGS(const char *filename, PPMImageGS *img)
{
  ...
  if (!fp) {
    fprintf(stderr, "Unable to open file '%s'201401086\n", filename);
    exit(1);
  }
...
// 	par_start=omp_get_wtime();
	...
	shared(rows,columns,im2,impimage) \
	private (x_toreadfrom,y_toreadfrom,distcen,i,j,idx2) 
	for (i=0;i<rows;i++)
	{
		for(j=0;j<columns;j++)
		{
...
int main(int argc, char* argv[]){
	...
	writePPM("warpedimage_parallel.ppm",warped_image);
	return 0;
}
