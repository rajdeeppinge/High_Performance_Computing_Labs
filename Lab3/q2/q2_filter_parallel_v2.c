...
PPMImage *filter(PPMImage *impimage)
{
	...
	shared(rows,columns,filtered_image,impimage) \
	private (i,j,ri,rj,idx,idx2,neighbourCount,temp,r,g,b)
	for (i=0;i<rows;i++)
	{
		for(j=0;j<columns;j++)
		{
			idx2= columns*i + j;
			//double r[num_pixels_block],g[num_pixels_block],b[num_pixels_block];
			//double *r = (double *) malloc(num_pixels_block*sizeof(double));
			//double *g = (double *) malloc(num_pixels_block*sizeof(double));
			//double *b = (double *) malloc(num_pixels_block*sizeof(double));
			neighbourCount = 0;
			//printf("%d %d %d\n", idx2, i, j);
...
int main(int argc, char* argv[]){
	...
	printf("Time: %f, Fraction of parallelizable code(p) = %f\n", (end-start), (end-start)/parallel_time);


	writePPM("filtered_lena_parallel.ppm",filtered_image);
	return 0;
}
